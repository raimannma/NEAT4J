...
public enum Mutation {
  ...
  SUB_NODE(false) {
    ...
    public void mutate(final Network network) {
      ...
      network.remove(network.nodes.get((int) Math.floor(Math.random() * (network.nodes.size() - network.output - network.input) + network.input)));
    }
  },
  ADD_CONN(false) {
    @Override
  ...
  MOD_ACTIVATION(Activation.values()) {
    ...
    public void mutate(final Network network) {
      ...
      final int index = (int) Math.floor(Math.random() * (network.nodes.size() - (this.mutateOutput ? 0 : network.output) - network.input) + network.input);
  ...
  SWAP_NODES(true) {
    ...
    public void mutate(final Network network) {
      ...
      int index = (int) Math.floor(Math.random() * (network.nodes.size() - (this.mutateOutput ? 0 : network.output) - network.input) + network.input);
      final Node node = network.nodes.get(index);
      index = (int) Math.floor(Math.random() * (network.nodes.size() - (this.mutateOutput ? 0 : network.output) - network.input) + network.input);
